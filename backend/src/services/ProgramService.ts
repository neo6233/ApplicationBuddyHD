  ...
  private scoreCatalogItem(item: Program, data: ProgramSearchFilters): number {
    ...
    if (countryText) {
      if (countryMatches(item.country, countryText)) {
        total += 20;
      } else if (item.countries.some((country: string) => countryMatches(country, countryText))) {
        total += 5;
      } else {
        total -= 8;
      }
    ...
    if (item.fields.some((field: string) => includesAny(interestsText, [field]))) {
      ...
      if (
        ...
        total += 22;
      } else if (inferredField && item.fields.some((field: string) => includesAny(normalize(field), [inferredField]))) {
        total += 18;
