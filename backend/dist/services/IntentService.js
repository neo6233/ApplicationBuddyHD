...
const getRecentText = (history, limit = 8) => history
    .slice(-limit)
    .map(m => m.content)
    .join(' ');
const getLastAssistantMessage = (history) => [...history].reverse().find(message => message.role === 'assistant')?.content || '';
...
const extractLevel = (text) => extractFirstMatch(text, LEVEL_PATTERNS);
const extractField = (text) => extractFirstMatch(text, FIELD_PATTERNS);
const extractCountry = (text) => extractFirstMatch(text, COUNTRY_PATTERNS);
const extractQualification = (text) => {
    const match = text.match(/\b(high school|secondary school|12th|10th|diploma|certificate|bachelor(?:'s)?|master(?:'s)?|phd|doctorate)\b/i);
    return match?.[1]
        ? match[1]
...
const extractEnglishScore = (text) => {
    const match = text.match(/\b(ielts|pte|toefl|duolingo)\b[^0-9]*?(\d{1,3}(?:\.\d{1,2})?)?/i);
    if (match?.[1]) {
...
const getMergedCourseSlots = (message, history) => {
...
const getEligibilitySlots = (history) => {
...
const geminiDecision = (systemPrompt, maxOutputTokens = 180, temperature = 0.5) => ({
    type: 'gemini',
    systemPrompt,
    maxOutputTokens,
    temperature,
});
const courseReply = (slots) => {
    if (!slots.level) {
        return geminiDecision(`You are ARIA, an applicant buddy helping a student pick a course.
Ask only one short question.
We still need to know whether they want UG, PG, diploma, or PhD programs.
Be warm, natural, and concise.`, 90, 0.6);
    }
    if (!slots.field) {
        return geminiDecision(`You are ARIA, an applicant buddy helping a student pick a course.
Ask only one short question.
...
Keep the reply brief and natural.`, 90, 0.6);
    }
...
Keep it brief and helpful.`, 90, 0.6);
    }
    if (!slots.score) {
        return geminiDecision(`You are ARIA, an applicant buddy helping a student pick a course.
Ask only one short question.
...
Keep the reply short and conversational.`, 90, 0.6);
    }
    return {
        type: 'course_catalog',
        courseQuery: slots,
...
const eligibilityReply = (slots) => {
    if (!slots.qualification) {
        return geminiDecision(`You are ARIA, an applicant buddy helping check eligibility.
Ask only one short question.
We still need the student's highest qualification.
Keep it natural and concise.`, 90, 0.6);
    }
    if (!slots.score) {
        return geminiDecision(`You are ARIA, an applicant buddy helping check eligibility.
Ask only one short question.
...
Keep the reply short and natural.`, 90, 0.6);
    }
...
Keep the reply warm and concise.`, 90, 0.6);
    }
    return {
        type: 'gemini',
...
exports.IntentService = {
    ...
    buildDecision(message, history) {
        ...
        if (intent === 'greeting') {
            return geminiDecision(`You are ARIA, an applicant buddy.
...
End with one simple question.`, 110, 0.7);
        }
        if (intent === 'thanks') {
            return geminiDecision(`You are ARIA, an applicant buddy.
Reply to the user's thanks in one short, warm sentence.
Then ask one helpful follow-up question about what they want next.`, 90, 0.6);
        }
        if (intent === 'farewell') {
            return geminiDecision(`You are ARIA, an applicant buddy.
Reply with a short, friendly goodbye message.
Do not ask more than one question, and it is okay to ask none.`, 80, 0.5);
        }
