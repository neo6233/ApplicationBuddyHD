...
const hasBachelorQualification = (text) => includesAny(text, ['bachelor', "bachelor's", 'bachelors', 'btech', 'b.tech', 'b.sc', 'bsc', 'bba', 'b.e', 'graduation', 'graduate', 'b.a', 'b.com']);
...
const inferLevel = (q) => {
    ...
    if (includesAny(t, ['diploma', 'certificate', 'polytechnic']))
        return 'Diploma';
    if (hasBachelorQualification(t) || includesAny(t, ['undergraduate', 'ug']))
        return 'UG';
    if (hasSchoolQualification(t))
        return 'UG';
...
const inferNextLevel = (qualification) => {
    const t = normalize(qualification);
    if (hasBachelorQualification(t) || includesAny(t, ['undergraduate']))
        return 'PG';
    if (includesAny(t, ['master', 'msc', 'mtech', 'mba', 'post graduate', 'postgraduate']) || /\b(?:ma|pg)\b/i.test(t))
        return 'PG';
    if (includesAny(t, ['diploma', 'certificate', 'polytechnic']))
...
const scoreCatalogItem = (item, data) => {
    ...
    if (schoolLevelOnly && item.level === 'PG')
        return -999;
    if (hasBachelorQualification(qt) && item.level !== 'PG')
        return -999;
    if (level === 'PG' && (item.level === 'UG' || item.level === 'Diploma'))
        return -999;
    let total = 15;
    if (ct) {
        if (countryMatches(item.country, ct))
            total += 20;
        else if (item.countries.some(c => countryMatches(c, ct)))
            total += 5;
        else
            total -= 8;
    }
    if (nextLevel === item.level)
        total += 20;
    else if (level === item.level || level === 'Any')
        total += 10;
